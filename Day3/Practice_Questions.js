...
let fh = cel.map(x => (x*1.8) + 32);
console.log(`After conversion to Fahrenheit: ${fh}`);
...
const fil = users.filter(x => x.age >= 18);
console.log("Users above 18 are:",fil);
...
function morethan(str){
    console.log("Elements with more than 5 characters",str.filter(x => x.length > 5));
}

morethan(["Hello","Hi","Krishna","Jha","Welcome"]);
...
console.log("Sum:",prices.reduce((sum,current) => sum + current,0));
...
console.log("Number of Apples are:",count); 
...
let mod = numbers.filter(x => (x%2) == 0).map(x => x * x);

console.log("After filteration and modification:",mod);
...
let Products = [{ id: 1, title: 'Laptop' }, { id: 2, title: 'Mouse' }];

let title = Products.map(x => x.title);
console.log("Products Titles:",title);
