...
const rawFlights = ["London-Paris:150", "New York-Tokyo:invalid", "Dubai-Mumbai:450", "Berlin-Rome:95"];
let substr = rawFlights.map((p) =>{
    const [route,priceStr] = p.split(":");
...
}).filter((p) => p.price >= 100 && p.price <= 500).sort((a,b) => a.price - b.price);

let str = JSON.stringify(substr);
console.log(str);
...
const result = rawMovies
  .map((movie) => {
    const [name, genre, viewsStr] = movie.split("|");

    let views = Number(viewsStr);
    if (isNaN(views)) {
      views = 0;
    }
